...
def parse_jig_config(data: dict[str, Any]) -> JigConfig:
    ...
    if scales is None:
        return JigConfig(jig_id=jig_id, vendor=vendor, product=product, timezone=timezone)
    ...
    return JigConfig(jig_id=jig_id, vendor=vendor, product=product, timezone=timezone, adc_scales=scales)
